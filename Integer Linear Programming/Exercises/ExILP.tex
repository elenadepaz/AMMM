...
Minimize the total time required to assist all patients:
\[\min \max_{i \in N} (s_i + q_i)\]

We add a new variable \(z\) to represent \(\max_{i \in N} (s_i + q_i)\) and we minimize \(z\):
\[\min z\] 

\[\text{subject to } s_i + q_i \leq z \quad \forall i \in N\]
